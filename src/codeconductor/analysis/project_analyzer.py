...
class ProjectAnalyzer:
    ...
    def scan_fastapi_routes(self, project_path: str) -> list[dict[str, Any]]:
        ...
        # FAST SCAN: Only scan specific files we know contain FastAPI routes
        priority_files = [
            "test_fastapi_app.py",
            "simple_test.py",
        ...
        for file_path in self.project_path.rglob("*.py"):
            ...
            # Only scan priority files or files with FastAPI keywords
            if file_path.name in priority_files or any(
                keyword in str(file_path).lower()
                for keyword in ["app", "api", "route", "main", "server", "fastapi"]
            ):
