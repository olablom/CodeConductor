...
class MemoryWatchdog:
    ...
    async def _handle_emergency_cleanup(self, vram_percent: float):
        ...
        logger.warning(f"🚨 EMERGENCY: VRAM usage {vram_percent:.1f}% > {self.thresholds.emergency_percent}%")
    ...
    async def _handle_critical_cleanup(self, vram_percent: float):
        ...
        logger.warning(f"⚠️ CRITICAL: VRAM usage {vram_percent:.1f}% > {self.thresholds.critical_percent}%")
        ...
        if self.consecutive_high_usage >= self.max_consecutive_high_usage:
            ...
            logger.warning(f"🧹 Triggering critical cleanup after {self.consecutive_high_usage} consecutive high readings")
    ...
    async def _handle_warning_cleanup(self, vram_percent: float):
        ...
        self.consecutive_high_usage += 1
        logger.info(f"⚠️ WARNING: VRAM usage {vram_percent:.1f}% > {self.thresholds.warning_percent}%")
        
        # Only trigger cleanup if we've had consecutive high usage
        if self.consecutive_high_usage >= self.max_consecutive_high_usage:
            self.cleanup_triggers += 1
            logger.info(f"🧹 Triggering warning cleanup after {self.consecutive_high_usage} consecutive high readings")
...
def get_memory_watchdog() -> Optional[MemoryWatchdog]:
