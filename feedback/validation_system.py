...
class CodeValidator:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                for decorator in node.decorator_list:
                    if self._get_decorator_name(decorator) in ["app.get", "app.post", "app.put", "app.delete", "router.get", "router.post", "router.put", "router.delete"]:
                        has_routes = True
                        break
    ...
    def _calculate_compliance(self, tree, code: str) -> Dict[str, bool]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                for decorator in node.decorator_list:
                    if self._get_decorator_name(decorator) in ["app.get", "app.post", "router.get", "router.post"]:
    ...
    def _generate_suggestions(self, compliance: Dict[str, bool], issues: List[str], task_description: str) -> List[str]:
...
def validate_cursor_output(generated_code: str, task_description: str, project_path: str) -> ValidationResult:
    """
