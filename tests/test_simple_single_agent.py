...
class SimpleSingleAgentTester:
    ...
    def _test_binary_search(self, code: str) -> bool:
        ...
        try:
            ...
            if binary_search:
                ...
                return (binary_search(arr, 7) == 3 and 
                       binary_search(arr, 1) == 0 and
                       binary_search(arr, 15) == 7)
        except:
            pass
        return False
    ...
    def _test_rest_api(self, code: str) -> bool:
        ...
        return ("@app.route" in code or 
                "def login" in code or 
                "def register" in code or
                "flask" in code.lower() or
                "fastapi" in code.lower())
    
    async def run_single_test(self, test_name: str, prompt: str, timeout: float = 60.0) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await asyncio.wait_for(
                agent.generate_response(prompt, timeout=timeout),
                timeout=timeout
            )
    ...
    def save_results(self):
        ...
        summary = {
            ...
            "code_quality_metrics": {
                "avg_code_length": sum(r.get("validation", {}).get("code_length", 0) for r in self.results) / len(self.results),
                "syntax_success_rate": sum(1 for r in self.results if r.get("validation", {}).get("syntax_check", False)) / len(self.results) * 100
            }
        }
    ...
    def print_summary(self):
        ...
        for test_type, stats in type_stats.items():
            ...
            print(f"  {test_type}: {stats['successful']}/{stats['total']} ({success_rate:.1f}%) - {avg_time:.1f}s avg")
        
        # Code quality metrics
        print("\n📈 Code Quality Metrics:")
        syntax_success = sum(1 for r in self.results if r.get("validation", {}).get("syntax_check", False))
        avg_code_length = sum(r.get("validation", {}).get("code_length", 0) for r in self.results) / total_tests
        print(f"  Syntax success rate: {syntax_success}/{total_tests} ({syntax_success/total_tests*100:.1f}%)")
