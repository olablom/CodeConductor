...
async def test_rag_system():
    ...
    for i, result in enumerate(results):
        print(f"   Result {i+1}: {result['metadata'].get('filename', 'Unknown')} (score: {result['relevance_score']:.3f})")
...
"""
    rag.add_document("test_fibonacci", test_content, {
    ...
    for i, doc in enumerate(context):
        print(f"   Context {i+1}: {doc['metadata'].get('filename', 'Unknown')} (score: {doc['relevance_score']:.3f})")
